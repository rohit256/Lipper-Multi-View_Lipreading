...
respath = '../results/speaker_independent/3_View/'
weight_path = join(respath,'weights/')
datapath = '../../lipsync/dataset/numpy_datasets/'

def savedata(Ytr, Ytr_pred, Yte, Yte_pred,v1,v2,v3):
    respath_view = join(respath,'View='+str(v1)+','+str(v2)+','+str(v3)+'/')
    ...
    nameoffile = 'STCNN'+'_'+ speakerlist +'_'+str(v1)+','+str(v2)+','+str(v3)+'_'+str(SR)+'_'+str(NFRAMES)
...
def build_model(net_out):
    ...
    # third input model
    visible3 = Input(shape=(CHANNELS*COLORS,FRAME_ROWS,FRAME_COLS))

    conv31 = (Convolution2D(32, 3, border_mode='same',data_format='channels_first',
                            init='he_normal',kernel_regularizer=l2(reg)))(visible3)
    batch31 = (BatchNormalization())(conv31)
    LR31 = (LeakyReLU())(batch31)
    MP31 = LR31
    DO31 = MP31

    conv32 = (Convolution2D(64, 3, padding='same', data_format='channels_first',
                            init='he_normal',kernel_regularizer=l2(reg)))(DO31)
    batch32 = (BatchNormalization())(conv32)
    LR32 = (LeakyReLU())(batch32)
    MP32 = (AveragePooling2D(nb_pool,data_format='channels_first'))(LR32)
    DO32 = MP32

    conv33 = (Convolution2D(64, 3, padding='same', data_format='channels_first',
                            init='he_normal',kernel_regularizer=l2(reg)))(DO32)
    batch33 = (BatchNormalization())(conv33)
    LR33 = (LeakyReLU())(batch33)
    MP33 = LR33
    DO33 = MP33

    conv34 = (Convolution2D(128, 3, padding='same', data_format='channels_first',
                            init='he_normal',kernel_regularizer=l2(reg)))(DO33)
    batch34 = (BatchNormalization())(conv34)
    LR34 = (LeakyReLU())(batch34)
    MP34 = LR34
    DO34 = MP34

    conv35 = (Convolution2D(128, 3, padding='same', data_format='channels_first',
                            init='he_normal',kernel_regularizer=l2(reg)))(DO34)
    batch35 = (BatchNormalization())(conv35)
    LR35 = (LeakyReLU())(batch35)
    MP35 = LR35
    DO35 = MP35
    ...
    # merge input models
    merge = concatenate([flat11, flat21,flat31])
    ...
    model = Model(inputs=[visible1, visible2,visible3], outputs=L)
    print(model.summary())
...
def train_net(model1, Xtr_v1, Ytr_norm_v1, Xte_v1, Yte_norm_v1,
              Xtr_v2, Ytr_norm_v2, Xte_v2, Yte_norm_v2, Xtr_v3, Ytr_norm_v3, Xte_v3, Yte_norm_v3,
    ...
    history = model1.fit( [Xtr_v1, Xtr_v2, Xtr_v3], Ytr_norm_v1, shuffle=False,batch_size=batch_size,nb_epoch=epochs,
                         verbose=1, validation_data=([Xte_v1, Xte_v2, Xte_v3], Yte_norm_v1))
    return model1

def predict(model, X_v1, Y_means_v1, Y_stds_v1, X_v2, Y_means_v2, Y_stds_v2, X_v3, Y_means_v3, Y_stds_v3, batch_size=BATCH_SIZE):
    Y_pred = model.predict([X_v1,X_v2,X_v3], batch_size=batch_size, verbose=1)
		...
		for v1 in range(1,2):
			...
			for v2 in range(3,4):
				...
				for v3 in range(4,5):
					print('########################################')
					print('View = ',v3)
					viddata_v3, auddata_v3 = load_data(datapath,v3)
					print(viddata_v3.shape)
					(Xtr_v3,Ytr_v3), (Xte_v3, Yte_v3) = split_data(viddata_v3, auddata_v3)
					print(Xtr_v3.shape, Ytr_v3.shape)
					print(Xte_v3.shape, Yte_v3.shape)
					Xtr_v3, Ytr_norm_v3, Xte_v3, Yte_norm_v3, Y_means_v3, Y_stds_v3 = standardize_data(Xtr_v3, Ytr_v3, Xte_v3, Yte_v3)
					model = build_model(net_out)
					model = train_net(model, Xtr_v1, Ytr_norm_v1, Xte_v1, Yte_norm_v1,
							     Xtr_v2, Ytr_norm_v2, Xte_v2, Yte_norm_v2, Xtr_v3, Ytr_norm_v3, Xte_v3, Yte_norm_v3)
					model = train_net(model, Xtr_v1, Ytr_norm_v1, Xte_v1, Yte_norm_v1,
							     Xtr_v2, Ytr_norm_v2, Xte_v2, Yte_norm_v2, Xtr_v3, Ytr_norm_v3, Xte_v3, Yte_norm_v3,finetune = True)
					Ytr_pred = predict(model, Xtr_v1, Y_means_v1, Y_stds_v1,
							      Xtr_v2, Y_means_v2, Y_stds_v2, Xtr_v3, Y_means_v3, Y_stds_v3)
					Yte_pred = predict(model, Xte_v1, Y_means_v1, Y_stds_v1,
							      Xte_v2, Y_means_v2, Y_stds_v2,Xte_v3, Y_means_v3, Y_stds_v3)
					savedata(Ytr_v1, Ytr_pred, Yte_v1, Yte_pred,v1 ,v2,v3)
